...
metadata:
  ...
  scope: [root]
  auto_invoke:
    - "Breaking an SDD change into tasks (tasks phase)"
...
- If mode is `openspec`: Write `tasks.md` to the change folder.
- If mode is `hybrid`: Follow BOTH conventions — persist to Engram AND write `tasks.md` to filesystem.
