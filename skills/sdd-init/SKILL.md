...
Read and follow `~/vanskills/skills/_sdd-shared/persistence-contract.md` for mode resolution rules.
...
- If mode is `engram`:
  ...
  **Save project context**:
  ...
  (See `~/vanskills/skills/_sdd-shared/engram-convention.md` for full naming conventions.)
- If mode is `openspec`: Run full bootstrap with filesystem.
- If mode is `hybrid`: Run openspec bootstrap AND persist context to Engram.
...
Follow the same logic as the `skill-registry` skill:

1. Scan user skills: glob `*/SKILL.md` across ALL known skill directories. **User-level**: `~/vanskills/skills/`, `~/.claude/skills/`, `~/.config/opencode/skills/`, `~/.gemini/skills/`, `~/.cursor/skills/`, `~/.copilot/skills/`. **Project-level**: `.claude/skills/`, `.gemini/skills/`, `.agent/skills/`, `skills/`. Skip `sdd-*`, `_sdd-shared`, `skill-registry`. Deduplicate by name (project-level wins). Read frontmatter triggers.
