...
description: >
  SDD Orchestrator — coordinates Spec-Driven Development phases by delegating to sub-agents.
  Trigger: When user says "sdd", "/sdd-init", "/sdd-new", "/sdd-ff", "/sdd-continue", "/sdd-apply", "/sdd-verify", "/sdd-archive", or wants structured planning for a substantial change.
license: MIT
metadata:
  author: edoriban
  version: "2.0"
...
When launching a phase, require the sub-agent to read `~/vanskills/skills/sdd-{phase}/SKILL.md` first and return:
- `status`
- `executive_summary`
...
Shared convention files under `~/vanskills/skills/_sdd-shared/` provide full reference documentation (sub-agents have inline instructions — convention files are supplementary):
