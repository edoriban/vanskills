...
metadata:
  ...
  scope: [root]
  auto_invoke:
    - "Writing the technical design of an SDD change (design phase)"
...
- If mode is `openspec`: Write `design.md` to the change folder.
- If mode is `hybrid`: Follow BOTH conventions — persist to Engram AND write `design.md` to filesystem.
